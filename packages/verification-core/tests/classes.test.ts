...
describe("ageBand — calendar-year boundaries (P7-D13-A)", () => {
  ...
  it("age 17 => U18, age 18 => Elite boundary", () => {
    expect(ageBand(2009, run2026)).toBe("U18"); // 17
    expect(ageBand(2008, run2026)).toBe("Elite"); // 18
  });

  it("age 39 => Elite, age 40 => O40 boundary", () => {
    expect(ageBand(1987, run2026)).toBe("Elite"); // 39
...
describe("classOf", () => {
  it("combines gender and band", () => {
    expect(classOf(1990, "M", run2026)).toBe("M-Elite");
